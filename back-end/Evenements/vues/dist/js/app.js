...
function setSameHeights(e) {
	...
	var r = ResponsiveBootstrapToolkit.current();
	e.each(function () {
		...
		e.each(function () {
			$(this).css({
				height: "auto"
			}), t = Math.max(t, $(this).innerHeight())
		}), e.each(function () {
			var e = $(this).data("exclude") || "",
				o = e.split(","); - 1 === o.indexOf(r) && $(this).innerHeight(t)
		})
	})
}
...
config.ResponsiveBootstrapToolkitVisibilityDivs = {
	...
	highlight: function (e, r, t) {
		$(e).parents("div.form-group").addClass(r).removeClass(t)
	},
	unhighlight: function (e, r, t) {
		$(e).parents(".has-error").removeClass(r).addClass(t)
	},
	...
	var e = {
		...
		messages: {
			username: {
				required: "Merci d'entrer votre identifiant",
				email: "Merci d'entrer une adresse mail valide"
			},
			password: "Merci d'entrer votre mot de passe",
			agree: "Merci d'accepter notre charte"
		},
		invalidHandler: function () {
			animate({
				name: "shake",
	...
	var e = {
		...
		errorPlacement: function (e, r) {
			"firstname" == r.attr("name") || "lastname" == r.attr("name") ? (e.insertAfter($("#lastname").closest(".row")), r.parents("div.form-group").addClass("has-error")) : "password" == r.attr("name") || "retype_password" == r.attr("name") ? (e.insertAfter($("#retype_password").closest(".row")), r.parents("div.form-group").addClass("has-error")) : "agree" == r.attr("name") ? e.insertAfter("#agree-text") : e.insertAfter(r)
		},
		messages: {
			firstname: "Merci d'entrer vos nom et prénom",
			lastname: "Merci d'entrer vos nom et prénom",
			email: {
				required: "Merci d'entrer une adresse mail",
				email: "Merci d'entrer une adresse mail valide",
			},
	...
	var e = $(".item-actions-dropdown");
	$(document).on("click", function (r) {
		$(r.target).closest(".item-actions-dropdown").length || e.removeClass("active")
	}), $(".item-actions-toggle-btn").on("click", function (r) {
		r.preventDefault();
		var t = $(this).closest(".item-actions-dropdown");
		e.not(t).removeClass("active"), t.toggleClass("active")
	})
});
var npSettings = {
	easing: "ease",
...
NProgress.configure(npSettings), $(function () {
	...
	function t(e) {
		var r = "#dashboard-" + e + "-chart";
		switch ($(r).has("svg").length && $(r).empty(), e) {
		case "visits":
			o();
			break;
		case "downloads":
	...
	var e = null,
		r = "visits";
	$('a[data-toggle="tab"]').on("shown.bs.tab", function (o) {
		e = o.target, r = $(e).attr("href").replace("#", ""), t(r)
	...
	function e() {
		$(".dashboard-page .items .sparkline").each(function () {
			...
			if ($(this).data("data")) var r = $(this).data("data").split(",").map(function (e) {
				return e.indexOf(":") > 0 ? e.split(":") : e
			});
			else
				for (var r = [], t = 0; 17 > t; t++) r.push(Math.round(1000 * Math.random()));
			$(this).sparkline(r, {
	...
	function r() {
		...
		var r = e.closest(".sameheight-container");
		setSameHeights(r)
	}
	var e = $("#dashboard-sales-breakdown-chart");
	return e.length ? (r(), void $(document).on("themechange", function () {
		r()
	})) : !1
}), $(function () {
	function r() {
		e.empty();
		var r = config.chart.colorPrimary.toHexString(),
			t = tinycolor(config.chart.colorPrimary.toString()).darken(40).toHexString(),
			o = tinycolor(config.chart.colorPrimary.toString()).darken(10).toHexString(),
			a = {
				us: 2e3,
				ru: 2e3,
				gb: 1e4,
	...
	function e() {
		function s() {
			for (o.length && (o = o.slice(1)); o.length < n;) {
				var e = o.length ? o[o.length - 1] : 50,
					r = e + 10 * Math.random() - 5;
				o.push(0 > r ? 0 : r > 100 ? 100 : r)
			}
		...
		function h(e, r) {
			return e.toFixed(r.tickDecimals) + "€"
		}
		...
		function u(e) {
			$.plot($("#flot-line-chart-multi"), [{
				...
				yaxes: [{
					...
					alignTicksWithAxis: "right" == e ? 1 : null,
					position: e,
					tickFormatter: h
				}],
				legend: {
		...
		var l = $.plot(i, series, {
			grid: {
				...
				markings: function (e) {
					for (var r = [], t = e.xaxis, o = Math.floor(t.min); o < t.max; o += 2 * t.tickSize) r.push({
						xaxis: {
							from: o,
							to: o + t.tickSize
						},
	...
	return $("#morris-one-line-chart").length ? (e(), void $(document).on("themechange", function () {
		e()
	})) : !1
}), $(function () {
	return $(".form-control").length ? ($(".form-control").focus(function () {
	...
	$(".images-container").sortable({
		...
		onMove: function (e) {
			var r = $(e.related);
			return r.hasClass("add-image") ? !1 : void 0
		}
		...
		e.preventDefault(),
			$controlsButtonsStar.removeClass("active"), $controlsButtonsStar.parents(".image-container").removeClass("main"), $(this).addClass("active"), $(this).parents(".image-container").addClass("main")
	})
}), $(function () {
	function e() {
		$(".items-list-page .sparkline").each(function () {
			for (var e = $(this).data("type"), r = [], t = 0; 17 > t; t++) r.push(Math.round(100 * Math.random()));
			$(this).sparkline(r, {
				barColor: config.chart.colorPrimary.toString(),
				height: $(this).height(),
				type: e
	...
	return $("#select-all-items").length ? ($("#select-all-items").on("change", function () {
		var e = $(this).children(":checkbox").get(0);
		$(this).parents("li, th, tr, div, thead, tbody").siblings().find(":checkbox").prop("checked", e.checked).val(e.checked).change()
	}), e(), void $(document).on("themechange", function () {
		e()
	})) : !1
}), $(function () {
	$(".wyswyg").each(function () {
		var e = $(this).find(".toolbar"),
			r = $(this).find(".editor"),
			t = new Quill(r.get(0), {
				theme: "snow"
			});
...
$(function () {
	function n() {
		s().delay(config.delayTime).queue(function (e) {
			c(), l(), h(), $(document).trigger("themechange"), e()
		})
	}

	function s() {
		return e.themeName ? t.attr("href", "../Evenements/vues/dist/css/app-" + e.themeName + ".css") : t.attr("href", "../Evenements/vues/dist/css/app.css"), r.removeClass("header-fixed footer-fixed sidebar-fixed"), r.addClass(e.headerPosition), r.addClass(e.footerPosition), r.addClass(e.sidebarPosition), r
	}

	function l() {
		a.each(function () {
			$(this).data("theme") === e.themeName ? $(this).addClass("active") : $(this).removeClass("active")
		}), i.each(function () {
			var r = $(this).prop("name"),
				t = $(this).val();
			e[r] === t ? $(this).prop("checked", !0) : $(this).prop("checked", !1)
		})
	}

	function c() {
		config.chart.colorPrimary = tinycolor($ref.find(".chart .color-primary").css("color")), config.chart.colorSecondary = tinycolor($ref.find(".chart .color-secondary").css("color"))
	}

	function d() {
		var e = localStorage.getItem("themeSettings") ? JSON.parse(localStorage.getItem("themeSettings")) : {};
		return e.headerPosition = e.headerPosition || "", e.sidebarPosition = e.sidebarPosition || "", e.footerPosition = e.footerPosition || "", e
	}

	function h() {
		localStorage.setItem("themeSettings", JSON.stringify(e))
	}
	var e = d(),
		r = $("#app"),
		t = $("#theme-style"),
